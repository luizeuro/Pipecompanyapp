...
const DEFAULTS = {
  ...
  api_keys: () => ({ scope: 'read_write', created_at: now(), last_used_at: null, revoked_at: null }),
  reports: () => ({ client_id: null, kind: 'report', period_start: null, period_end: null, created_at: now() }),
}

const PRIMARY_KEY = { app_state: 'key' }
const UNIQUE = { users: ['email'], api_keys: ['key_hash'] }
// Tabelas filhas apagadas junto com o cliente (on delete cascade no SQL).
const CASCADE_FROM_CLIENTS = ['account_snapshots', 'optimizations', 'pendencias', 'alerts', 'contacts', 'interactions', 'reports']
