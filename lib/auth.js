...
import { isApiKey, verifyApiKey } from './apiKeys.js'
...
// Agentes (Hermes etc.) entram com "Authorization: Bearer pc_live_..." e viram
// um usuário de papel "agent" — o que eles podem fazer é limitado em api/index.js.
export async function requireAuth(req, res, next) {
  const header = req.get('authorization') || ''
  const bearer = header.startsWith('Bearer ') ? header.slice(7).trim() : null
  if (isApiKey(bearer)) {
    const key = await verifyApiKey(bearer)
    if (!key) return res.status(401).json({ error: 'Chave de API inválida ou revogada.' })
    req.user = { id: null, name: `${key.name} (agente)`, email: null, role: 'agent', scope: key.scope, key_id: key.id }
    return next()
  }
